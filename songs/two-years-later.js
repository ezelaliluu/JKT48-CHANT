const songTwoYearsLater = {
    id: 9,
    title: "Two Years Later",
    cover: "images/pajama-drive.png", 
    src: "song/two-years-later.mp3",
    lyrics: [
        {
            time: 5,
            text: "",
            chant: "urya oi"
        },
        {
            time: 5.8,
            text: "",
            chant: "urya oi"
        },
        {
            time: 6.6,
            text: "",
            chant: "urya oi"
        },
        {
            time: 7.4,
            text: "",
            chant: "urya oi"
        },
        {
            time: 8.3,
            text: "",
            chant: "aaaaaaaaa"
        },
        {
            time: 9.7,
            text: "",
            chant: "yossha ikuzo"
        },
        {
            time: 11.2,
            text: "",
            chant: "taiga"
        },
        {
            time: 12,
            text: "",
            chant: "faiya"
        },
        {
            time: 12.9,
            text: "",
            chant: "saiba"
        },
        {
            time: 13.8,
            text: "",
            chant: "faiba"
        },
        {
            time: 14.7,
            text: "",
            chant: "daiba"
        },
        {
            time: 15.6,
            text: "",
            chant: "baiba"
        },

        {
            time: 16.5,
            text: "",
            chant: "jya jya"
        },



        // --- INTRO & VERSE 1 (Mundur ke detik 20 biar gak colong start) ---
        {
            time: 17,
            text: "Kudorong pintu, yang susah dibuka",
            chant: "syalalalala, syalalalala"
        },
        {
            time: 21,
            text: "Ke dalam tempat yang tak berpenghuni",
            chant: "jya jya jya wakacipaw weu aweu aweu aweu"
        },
        { time: 28, text: "Besok (di sini) Ayo (bertemu)", isChant: false },
        { time: 31, text: "E-mail (sebaris) Yang datang (mendadak)", isChant: false },

        // --- VERSE 2 ---
        { time: 34, text: "Kaca mozaik itu sangat nostaljik", isChant: false },
        {
            time: 40,
            text: "Sebelum diriku, menjadi dewasa",
            chant: "syalalalala, syalalalala"
        },
        {
            time: 45,
            text: "Tak bisa cintaimu saat dimanjakan",
            chant: "jya jya jya wakacipaw weu aweu aweu aweu"
        },
        { time: 51, text: "Cinta (lama di) Lubuk (hatiku)", isChant: false },
        { time: 53, text: "Sedih (nan manis) Tinggalkan (duri)", isChant: false },
        { time: 57, text: "Penanda dari kenangan", isChant: false },

        // --- PRE-CHORUS 1 ---
        { time: 61, text: "Kenapa... dua tahun berlalu...", isChant: false },
        { time: 67, text: "Kenapa... membuatku berbalik...", isChant: false },
        { time: 72, text: "Beri tahu...", isChant: false },

        // --- CHORUS 1 ---
        { time: 76, text: "Maafkanlah Tuhan, ampunilah diriku ini", isChant: false },
        { time: 80, text: "Yang tidak percaya 'kan cinta", isChant: false },
        { time: 83, text: "Aku adalah wanita yang jahat, ahhh...", isChant: false },
        { time: 88, text: "Sesatkan orang yang ditemui selanjutnya dengan kesedihan", isChant: false },
        { time: 94, text: "Sesuatu yang penting bagiku telah menghilang, ahhh...", isChant: false },

        {
            time: 106,
            text: "",
            chant: "urya oi"
        },
        {
            time: 106.8,
            text: "",
            chant: "urya oi"
        },
        {
            time: 107.6,
            text: "",
            chant: "urya oi"
        },
        {
            time: 108.4,
            text: "",
            chant: "urya oi"
        },
        {
            time: 108.8,
            text: "",
            chant: "aaaaaaaaa"
        },
        {
            time: 110,
            text: "",
            chant: "yossha ikuzo"
        },
        {
            time: 112,
            text: "",
            chant: "tora"
        },
        {
            time: 112.7,
            text: "",
            chant: "hi"
        },
        {
            time: 113.3,
            text: "",
            chant: "jinzou"
        },
        {
            time: 113.9,
            text: "",
            chant: "seni"
        },
        {
            time: 114.6,
            text: "",
            chant: "ama"
        },
        {
            time: 115.1,
            text: "",
            chant: "shindou"
        },
        {
            time: 115.6,
            text: "",
            chant: "kasen tobi jokyo"
        },

        // --- VERSE 3 ---
        {
            time: 116,
            text: "Baiknya bertemu dengan muka apa?",
            chant: "syalalalala, syalalalala"
        },
        {
            time: 122,
            text: "Karena ku bukan aku yang waktu dulu",
            chant: "jya jya jya wakacipaw weu aweu aweu aweu"
        },
        { time: 128, text: "Ada (tujuan) Tak bisa (pergi)", isChant: false },
        { time: 131, text: "Mungkinku (memakai) Bohong yang (pandai)", isChant: false },
        { time: 135, text: "Ku bingung menulis e-mail balasan", isChant: false },

        // --- VERSE 4 ---
        {
            time: 139,
            text: "Di sisi ujung bangku panjang itu",
            chant: "syalalalala, syalalalala"
        },
        {
            time: 144,
            text: "Kuhela nafasku, hilangkan semua perandaian",
            chant: "jya jya jya wakacipaw weu aweu aweu aweu"
        },
        { time: 151, text: "Sampai (sekarang) Kuingin (cintai)", isChant: false },
        { time: 154, text: "Dirimu (selalu) Ego tak (terucap)", isChant: false },
        { time: 157, text: "Waktu itu sangat kejam", isChant: false },

        // --- PRE-CHORUS 2 ---
        { time: 163, text: "Hai... kenapa manusia...", isChant: false },
        { time: 168, text: "Hai... sadari kesalahan... yang dibuat...", isChant: false },

        // --- CHORUS 2 ---
        { time: 174, text: "Maafkanlah Tuhan, diriku yang terlalu bodoh", isChant: false },
        { time: 180, text: "Tak bisa menepati janji", isChant: false },
        { time: 183, text: "Kau adalah orang yang baik dahulu, ahhh...", isChant: false },
        { time: 188, text: "Tapi pada jari manisku ada seseorang yang baru", isChant: false },
        { time: 194, text: "Sudah, tak bisa kembali ke masa itu, ahhh...", isChant: false },

        // --- CHORUS 3 / FINAL CHORUS ---
        { time: 210.5, text: "Maafkanlah Tuhan, ampunilah diriku ini", isChant: false },
        { time: 214, text: "Yang tidak percaya 'kan cinta", isChant: false },
        { time: 217, text: "Aku adalah wanita yang jahat, ahhh...", isChant: false },
        { time: 223, text: "Sesatkan orang yang ditemui selanjutnya dengan kesedihan", isChant: false },
        { time: 229, text: "Sesuatu yang penting bagiku telah menghilang, ahhh...", isChant: false }
    ]
};